...
mbayes_drift_condition_congruency_interact_results <- lapply(1:5, function(x) summarizebrms(mbayes_drift_condition_congruency_interact[[x]], conf.method = "HPDinterval", effect = "conditionEC:congruentEC"))

manuscriptformat <- data.table(results = sapply(1:5, function(x) mbayes_drift_condition_congruency_interact_results[[x]][effect == "manuscriptformat", result]))
manuscriptformat
mbayes_drift_condition_congruency_interact_results[[5]]

tableformat <- lapply(1:5, function(x) formattable(mbayes_drift_condition_congruency_interact_results[[x]]))
tableformat
