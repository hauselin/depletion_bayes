...
prior_coef <- expectedBeta(expected_d = -prior_informed_cohensd,
                           sd1 = stroop[condition == "control", sd(acc, na.rm = T)], # REMEMBER TO CHANGE VARIALBE!
                           sd2 = stroop[condition == "deplete", sd(acc, na.rm = T)])
prior_coef
get_prior(acc ~ conditionEC + (1 | study/pNo), stroop[study == 1])
...
mbayes_acc_condition_congruency_interact <- vector(mode = "list", length = 5)
for (s in 1:4) {
    mbayes_acc_condition_congruency_interact[[s]] <- brm(acc ~ conditionEC * congruentEC + (1 | pNo), data = stroop[study == s],
                             # family = lognormal(),
                             cores = nchains, chains = nchains, sample_prior = TRUE, save_all_pars = TRUE, prior = priors, iter = samples,
                             file = glue("brms_models/mbayes_acc_condition_congruency_interact_study{s}"),
                             control = list(adapt_delta = 0.99))
}

# fit 3-level model
mbayes_acc_condition_congruency_interact[[5]] <- brm(acc ~ conditionEC * congruentEC + (1 | study/pNo), data = stroop,
                             # family = lognormal(),
                             cores = nchains, chains = nchains, sample_prior = TRUE, save_all_pars = TRUE, prior = priors, iter = samples * 3,
                             file = "brms_models/mbayes_acc_condition_congruency_interact_study_all",
                             control = list(adapt_delta = 0.99))
...
mbayes_acc_condition_congruency_interact_results <- lapply(1:5, function(x) summarizebrms(mbayes_acc_condition_congruency_interact[[x]], conf.method = "HPDinterval", effect = "congruentEC"))
...
mbayes_acc_condition_congruency_interact_results <- lapply(1:5, function(x) summarizebrms(mbayes_acc_condition_congruency_interact[[x]], conf.method = "HPDinterval", effect = "conditionEC:congruentEC"))
...
# fit model for each study (2-level model)
mbayes_acc_condition_congruency_interact_nocondition <- vector(mode = "list", length = 5)
for (s in 1:4) {
    mbayes_acc_condition_congruency_interact_nocondition[[s]] <- brm(acc ~ congruentEC + conditionEC:congruentEC + (1 | pNo), data = stroop[study == s],
                                                         # family = lognormal(),
                                                         cores = nchains, chains = nchains, sample_prior = TRUE, save_all_pars = TRUE, prior = priors, iter = samples,
                                                         file = glue("brms_models/mbayes_acc_condition_congruency_interact_study{s}_nocondition"),
                                                         control = list(adapt_delta = 0.99))
}

# fit 3-level model
mbayes_acc_condition_congruency_interact_nocondition[[5]] <- brm(acc ~ congruentEC + conditionEC:congruentEC + (1 | study/pNo), data = stroop,
                                                     # family = lognormal(),
                                                     cores = nchains, chains = nchains, sample_prior = TRUE, save_all_pars = TRUE, prior = priors, iter = samples * 3,
                                                     file = "brms_models/mbayes_acc_condition_congruency_interact_study_all_nocondition",
                                                     control = list(adapt_delta = 0.99))

# bridge sampling bayes factors
compute_bfs(mbayes_acc_condition_congruency_interact, mbayes_acc_condition_congruency_interact_nocondition)
...
# bridge sampling bayes factors
compute_bfs(mbayes_acc_condition_congruency_interact, mbayes_acc_condition_congruency_interact_nointeraction)
